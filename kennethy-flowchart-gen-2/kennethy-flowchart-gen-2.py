...
def generate_flowchart(num_flowcharts, min_nodes, max_nodes):
    os.makedirs("kennethy-flowchart-gen-2/mermaid_txt", exist_ok=True)
    os.makedirs("kennethy-flowchart-gen-2/flowchart_img", exist_ok=True)
    paths = []
    ...
    for flowchart_index in range(1, num_flowcharts + 1):
        ...
        for node in range(num_nodes):
            ...
            if node == 0:
                ...
                leaf_to_end = random.choice(leaf)
                ...
                for l in leaf:
                    ...
                    choices_from_parents = getAllParents(node_name)
                    choices_from_parents = [i for i in choices_from_parents if l[0] not in parents[str(i[0])] + [i[0]]]
        ...
        mermaid_text_path = os.path.join(
            "kennethy-flowchart-gen-2/mermaid_txt",
...
def draw_flowchart(mermaid_text_paths):
    for mermaid_text_path in mermaid_text_paths:
        ...
        flowchart_image_path = os.path.join(
            "kennethy-flowchart-gen-2/flowchart_img",
            f"flowchart_{f_num}"
        )
...
num_flowcharts = 3  # Number of flowcharts to generate
min_nodes = 5  # Minimum number of nodes
max_nodes = 15  # Maximum number of nodes
mermaid_path = generate_flowchart(num_flowcharts, 5, max_nodes)
